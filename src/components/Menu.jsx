...
export default function Menu () {
  ...
  const getCocktailAt = (index) => {
    const adjustedIndex = (currentIndex + index + totalCocktails) % totalCocktails;
    return allCocktails[adjustedIndex];
  }
        ...
        {allCocktails.map((cocktail, index) => {
          ...
          return (
            <button
              key={cocktail.id}
              className={` 
                ${isActive ? 'text-white border-white' : 'text-white/50 border-white/50 hover:border-white/80'}`
              }
